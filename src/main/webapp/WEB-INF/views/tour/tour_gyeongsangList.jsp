		...
		<div style="width: 900px; height: 2000px; background-color: #eaeaea; overflow-y: scroll;">
				...
				<div class="hbox paging">
					<c:if test="${map.paging.prev }">
						<a href="${cpath }/tour_gyeongsang/${map.paging.begin - 1}?type=${param.type}&word=${param.word}">◀</a>
					</c:if>
				</div>
				<c:forEach begin="${map.paging.begin }" end="${map.paging.end }" var="i">
					<a href="${cpath }/tour_gyeongsang/${i}?type=${param.type}&word=${param.word}">
				...
				<c:if test="${map.paging.next }">
					<a href="${cpath }/tour_gyeongsang/${map.paging.end + 1}?type=${param.type}&word=${param.word}">▶</a>
				</c:if>
				<c:choose>
